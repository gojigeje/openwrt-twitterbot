...
twit_kissanime_main() {
  ...
  while [[ $num -lt $max ]]; do
    ...
    if [[ "$line" == *"Barakamon"* ]]; then
      status="[kissanime] Episode terbaru Anime Barakamon udah rilis..! Liat + Download di kissanime :D ~ cc. @gojigeje"
    fi
    if [[ "$line" == *"Glasslip"* ]]; then
      status="[kissanime] Episode terbaru Anime Glasslip udah rilis..! Liat + Download di kissanime :D ~ cc. @gojigeje"
    fi
